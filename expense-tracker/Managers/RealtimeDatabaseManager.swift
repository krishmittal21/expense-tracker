...
class RealtimeDatabaseManager {
    ...
    var typingStatusHandle: DatabaseHandle? // Store the observer handle
    var lastObservedGroupId: String?
    var queryHandles: [IndexPath: UInt] = [:]
}
...
extension RealtimeDatabaseManager {
    ...
    func observe<T: Codable>(at path: String, eventType: DataEventType, condition: (key: String, value: Any)? = nil, completion: @escaping (Result<(T, String), Error>) -> Void) {
        guard !path.isEmpty else {
            completion(.failure(NSError(domain: "InvalidInput", code: 400, userInfo: [NSLocalizedDescriptionKey: "Path and Document ID cannot be empty."])))
            return
        }
        ...
        query.observe(eventType) { snapshot in
            ...
            do {
                ...
                let jsonData = try JSONSerialization.data(withJSONObject: data, options: [])
                let decodedObject = try JSONDecoder().decode(T.self, from: jsonData)
                completion(.success((decodedObject, snapshot.key ?? "")))
            } catch {
                completion(.failure(error))
            }
        }
