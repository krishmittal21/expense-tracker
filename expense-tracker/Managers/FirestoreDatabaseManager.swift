...
extension FirestoreDatabaseManager {
    ...
    func queryDocuments(collectionPath: String, filters: [(field: String, isEqualTo: Any)], completion: @escaping (Result<[[String: Any]], Error>) -> Void) {
        ...
        // Apply multiple whereField filters
        for filter in filters {
            query = query.whereField(filter.field, isEqualTo: filter.isEqualTo)
        }
    ...
    // Experimental -> Do not use (to be tested)
    func queryDocuments(collectionPath: String, conditions: [(field: String, isEqualTo: Any)], completion: @escaping (Result<[[String: Any]], Error>) -> Void) {
        guard !collectionPath.isEmpty else {
            completion(.failure(NSError(domain: "InvalidInput", code: 400, userInfo: [NSLocalizedDescriptionKey: "Path and Document ID cannot be empty."])))
            return
        }
        ...
        for condition in conditions {
            query = query.whereField(condition.field, isEqualTo: condition.isEqualTo)
        }
    ...
    func postDocument(collectionPath: String, data: [String: Any], documentId:String = UUID().uuidString, completion: @escaping (Result<String, Error>) -> Void) {
        guard !collectionPath.isEmpty else {
            completion(.failure(NSError(domain: "InvalidInput", code: 400, userInfo: [NSLocalizedDescriptionKey: "Path and Document ID cannot be empty."])))
            return
        }
